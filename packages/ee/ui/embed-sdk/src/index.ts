...
export enum yflowClientEventName {
...
export interface yflowClientInit {
  type: yflowClientEventName.CLIENT_INIT;
  data: Record<string, never>;
}
export interface yflowClientAuthenticationSuccess {
  type: yflowClientEventName.CLIENT_AUTHENTICATION_SUCCESS;
  data: Record<string, never>;
}
export interface yflowClientAuthenticationFailed {
  type: yflowClientEventName.CLIENT_AUTHENTICATION_FAILED;
  data: unknown;
}
...
export interface yflowClientConfigurationFinished {
  type: yflowClientEventName.CLIENT_CONFIGURATION_FINISHED;
  data: Record<string, never>;
}
export interface yflowClientShowConnectionIframe {
  type: yflowClientEventName.CLIENT_SHOW_CONNECTION_IFRAME;
  data: Record<string, never>;
}
export interface yflowClientConnectionNameIsInvalid {
  type: yflowClientEventName.CLIENT_CONNECTION_NAME_IS_INVALID;
  data: {
    error: string;
  };
}

export interface yflowClientConnectionPieceNotFound {
  type: yflowClientEventName.CLIENT_CONNECTION_PIECE_NOT_FOUND;
  data: {
    error: string
  };
}

export interface yflowClientRouteChanged {
  type: yflowClientEventName.CLIENT_ROUTE_CHANGED;
  data: {
    route: string;
  };
}
export interface yflowNewConnectionDialogClosed {
  type: yflowClientEventName.CLIENT_NEW_CONNECTION_DIALOG_CLOSED;
  data: { connection?: { id: string; name: string } };
}
export interface yflowBuilderHomeButtonClicked {
  type: yflowClientEventName.CLIENT_BUILDER_HOME_BUTTON_CLICKED;
  data: {
    route: string;
  };
}
...
export type yflowClientEvent =
  | yflowClientInit
  | yflowClientRouteChanged;

export enum yflowVendorEventName {
  VENDOR_INIT = 'VENDOR_INIT',
  VENDOR_ROUTE_CHANGED = 'VENDOR_ROUTE_CHANGED',
}

export interface yflowVendorRouteChanged {
  type: yflowVendorEventName.VENDOR_ROUTE_CHANGED;
  data: {
    vendorRoute: string;
  };
}

export interface yflowVendorInit {
  type: yflowVendorEventName.VENDOR_INIT;
  data: {
    hideSidebar: boolean;
...
class yflowEmbedded {
  ...
  _jwtToken = '';
  _resolveNewConnectionDialogClosed?: (result: yflowNewConnectionDialogClosed['data']) => void;
  ...
  private _setupInitialMessageHandler(targetWindow: Window, initialRoute: string, callbackAfterConfigurationFinished?: () => void) {
    const initialMessageHandler = (event: MessageEvent<yflowClientEvent>) => {
      if (event.source === targetWindow && event.origin === new URL(this._instanceUrl).origin) {
        switch (event.data.type) {
          case yflowClientEventName.CLIENT_INIT: {
            const apEvent: yflowVendorInit = {
              type: yflowVendorEventName.VENDOR_INIT,
              data: {
  ...
  private _createConfigurationFinishedListener = (targetWindow: Window, callbackAfterConfigurationFinished?: () => void) => {
    const configurationFinishedHandler = (event: MessageEvent<yflowClientConfigurationFinished>) => {
      if (event.data.type === yflowClientEventName.CLIENT_CONFIGURATION_FINISHED && event.source === targetWindow) {
  ...
  private _createAuthenticationFailedListener = (targetWindow: Window) => {
    const authenticationFailedHandler = (event: MessageEvent<yflowClientAuthenticationFailed>) => {
        if (event.data.type === yflowClientEventName.CLIENT_AUTHENTICATION_FAILED && event.source === targetWindow) {
  ...
  private _createAuthenticationSuccessListener = (targetWindow: Window) => {
    const authenticationSuccessHandler = (event: MessageEvent<yflowClientAuthenticationSuccess>) => {
      if (event.data.type === yflowClientEventName.CLIENT_AUTHENTICATION_SUCCESS && event.source === targetWindow) {
  ...
  async connect({ pieceName, connectionName, newWindow }: { 
    ...
    return this._addGracePeriodBeforeMethod({
      ...
      method: async () => {
        ...
        return new Promise<yflowNewConnectionDialogClosed['data']>((resolve, reject) => {
  ...
  navigate({ route }: { route: string }) {
    ...
    const event: yflowVendorRouteChanged = {
      type: yflowVendorEventName.VENDOR_ROUTE_CHANGED,
      data: {
  ...
  private _checkForClientRouteChanges = (source: Window) => {
    window.addEventListener(
      'message',
      (event: MessageEvent<yflowClientRouteChanged>) => {
        if (
          event.data.type ===
          yflowClientEventName.CLIENT_ROUTE_CHANGED &&
          event.source === source && 
  ...
  private _checkForBuilderHomeButtonClicked = (source: Window) => {
    window.addEventListener('message', (event: MessageEvent<yflowBuilderHomeButtonClicked>) => {
      if (event.data.type === yflowClientEventName.CLIENT_BUILDER_HOME_BUTTON_CLICKED && event.source === source) {
  ...
  extractyflowRouteFromUrl({ vendorUrl }: { vendorUrl: string }) {
  ...
  private _setConnectionIframeEventsListener(target: Window | HTMLIFrameElement ) {
    const connectionRelatedMessageHandler = (event: MessageEvent<yflowNewConnectionDialogClosed | yflowClientConnectionNameIsInvalid | yflowClientShowConnectionIframe | yflowClientConnectionPieceNotFound>) => {
      if (event.data.type) {
        switch (event.data.type) {
          case yflowClientEventName.CLIENT_NEW_CONNECTION_DIALOG_CLOSED: {
            if (this._resolveNewConnectionDialogClosed) {
          ...
          case yflowClientEventName.CLIENT_CONNECTION_NAME_IS_INVALID:
          case yflowClientEventName.CLIENT_CONNECTION_PIECE_NOT_FOUND: {
            this._removeEmbedding(target);
          ...
          case yflowClientEventName.CLIENT_SHOW_CONNECTION_IFRAME: {
            if (target instanceof HTMLIFrameElement) {
  ...
  private _logger() {
    return{
      log: (message: string, ...args: any[]) => {
        console.log(`yflow: ${message}`, ...args)
      },
      error: (message: string, ...args: any[]) => {
        console.error(`yflow: ${message}`, ...args)
      },
      warn: (message: string, ...args: any[]) => {
        console.warn(`yflow: ${message}`, ...args)
      }
    }
  }
...
(window as any).Yflow= new yflowEmbedded();
(window as any).yflowEmbedded = yflowEmbedded;
