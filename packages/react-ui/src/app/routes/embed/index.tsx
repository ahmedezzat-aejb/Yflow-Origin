...
import {
  yflowClientAuthenticationFailed,
  yflowClientAuthenticationSuccess,
  yflowClientConfigurationFinished,
  yflowClientEventName,
  yflowClientInit,
  yflowVendorEventName,
  yflowVendorInit,
  yflowVendorRouteChanged,
} from 'ee-embed-sdk';

const notifyVendorPostAuthentication = () => {
  const authenticationSuccessEvent: yflowClientAuthenticationSuccess = {
    type: yflowClientEventName.CLIENT_AUTHENTICATION_SUCCESS,
    data: {},
  };
  parentWindow.postMessage(authenticationSuccessEvent, '*');
  const configurationFinishedEvent: yflowClientConfigurationFinished = {
...
const handleVendorNavigation = ({ projectId }: { projectId: string }) => {
  const handleVendorRouteChange = (
    event: MessageEvent<yflowVendorRouteChanged>,
  ) => {
    if (
      event.source === parentWindow &&
      event.data.type === yflowVendorEventName.VENDOR_ROUTE_CHANGED
    ) {
...
const EmbedPage = React.memo(() => {
  ...
  const initState = (event: MessageEvent<yflowVendorInit>) => {
    if (
      event.source === parentWindow &&
      event.data.type === yflowVendorEventName.VENDOR_INIT
    ) {
      if (event.data.data.jwtToken) {
        ...
        mutateAsync(
          ...
          },
          {
            ...
            onError: (error) => {
              const errorEvent: yflowClientAuthenticationFailed = {
