import { CreateAICreditCheckoutSessionParamsSchema, UpdateAICreditsAutoTopUpParamsSchema } from '@yflow/ee-shared'
import { exceptionHandler, sleep } from '@yflow/server-shared'
import { AiCreditsAutoTopUpState, assertNotNullOrUndefined, isNil, PlatformPlan, tryCatch } from '@yflow/shared'
...
export const platformAiCreditsService = (log: FastifyBaseLogger) => ({
    ...
    async getUsage(platformId: string): Promise<APIKeyUsage> {
        ...
        const auth = await aiProviderService(log).getyflowProviderIfEnriched(platformId)
        if (isNil(auth)) {
    ...
    async aiCreditsPaymentSucceeded(platformId: string, amount: number, _paymentType: StripeCheckoutType): Promise<void> {
        const { apiKeyHash } = await aiProviderService(log).getOrCreateyflowProviderAuthConfig(platformId)
