...
import { AppSystemProp, ContainerType, DatabaseType, RedisType, SystemProp, WorkerSystemProp } from '@yflow/server-shared'
import { ApEdition, ApEnvironment, ExecutionMode, FileLocation, isNil, PieceSyncMode } from '@yflow/shared'
...
export const validateEnvPropsOnStartup = async (log: FastifyBaseLogger): Promise<void> => {
    ...
    if (environment !== ApEnvironment.TESTING && fileStorageLocation === FileLocation.S3) {
        try {
            ...
            throw new Error(JSON.stringify({
                ...
                docUrl: 'https://www.yflow.com/docs/install/configuration/overview#configure-s3-optional',
            }))
        }
    }
    ...
    if (!isNil(codeSandboxType)) {
        throw new Error(JSON.stringify({
            ...
            docUrl: 'https://www.yflow.com/docs/install/configuration/overview',
        }))
    }
    ...
    if (!isValidHexKey) {
        throw new Error(JSON.stringify({
            ...
            docUrl: 'https://www.yflow.com/docs/install/configuration/environment-variables',
        }))
    }
