...
export class ChangeExternalIdsForTables1747346473001 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const flowVersionIds = await queryRunner.query(
            'SELECT id FROM "flow_version" WHERE CAST("trigger" AS TEXT) LIKE \'%@yflow/piece-tables%\'',
        )
        log.info(
...
const updateVersionOfTablesStep = (
    ...
    if ((step.type === 'PIECE' || step.type === 'PIECE_TRIGGER') && (step as PieceStep).settings.pieceName === '@yflow/piece-tables') {
