import { assertNotNullOrUndefined } from '@yflow/shared'
...
export enum AppSystemProp {
    ...
    SBERBANK_MERCHANT_ID = 'SBERBANK_MERCHANT_ID',
    SBERBANK_WEBHOOK_SECRET = 'SBERBANK_WEBHOOK_SECRET',
    SBERBANK_BASE_URL = 'SBERBANK_BASE_URL',
    ...
    async getLatestRelease(): Promise<string> {
        try {
            ...
            const response = await axios.get<PackageJson>(
                'https://raw.githubusercontent.com/yflow/yflow/main/package.json',
                {
                    timeout: 5000,
                },
            )
