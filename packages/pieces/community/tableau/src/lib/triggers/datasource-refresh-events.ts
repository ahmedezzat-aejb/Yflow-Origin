import { createTrigger, TriggerStrategy, Property } from '@yflow/pieces-framework';
...
export const datasourceRefreshEventTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    const webhookData: WebhookData = {
      name: `yflow_Datasource_Refresh_${eventType}_${context.webhookUrl.substring(context.webhookUrl.lastIndexOf('/') + 1)}`,
      event: eventType,
