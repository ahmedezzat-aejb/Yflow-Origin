...
export const generateImageFromText = createAction({
	...
	props: {
		...
		promptText: Property.LongText({
			displayName: 'Prompt',
			description: 'Describe what you want to see in image (max 1000 characters)',
			required: true,
		}),
		...
		referenceImages: Property.Array({
			...
			properties: {
				uri: Property.ShortText({
					...
					description: 'HTTPS URL or data URI of reference image',
					required: true
				}),
	...
	async run({ auth, propsValue }) {
		// Zod validation - Fixed to include all required props
		await propsValidation.validateZod(propsValue, {
			...
			ratio: z.string().min(1, 'Image ratio is required'),
			...
			publicFigureThreshold: z.string().optional(),
		});
