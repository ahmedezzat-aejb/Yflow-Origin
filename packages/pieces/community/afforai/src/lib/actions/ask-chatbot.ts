...
import { Property, createAction } from '@yflow/pieces-framework';
import {
  HttpMethod,
  HttpRequest,
  httpClient,
...
export const askChatbotAction = createAction({
  ...
  props: {
    ...
    history: Property.Array({
      ...
      properties: {
        role: Property.StaticDropdown({
          displayName: 'Role',
          description: 'The role of message sender.',
          required: true,
          options: {
            disabled: false,
            options: [
    ...
    powerful: Property.Checkbox({
      displayName:
        'AI should search more deeply for information in given files ?',
      required: true,
    }),
