import { AppConnectionValueForAuthProperty, DynamicPropsValue, Property } from '@yflow/pieces-framework';
import { mondayClient } from './client';
import { MondayColumnType, MondayNotWritableColumnType } from './constants';
import { convertMondayColumnToYflowProp } from './helper';
        ...
        for (const column of columns) {
          if (!MondayNotWritableColumnType.includes(column.type)) {
            if (column.type === MondayColumnType.PEOPLE) {
              ...
              const prop = convertMondayColumnToYflowProp(column);
