import { DynamicPropsValue, Property } from '@yflow/pieces-framework';
...
export const kizeoFormsCommon = {
    ...
    const response = await httpClient.sendRequest<{ forms: KizeoFormsForms[] }>(
      {
        ...
        url: endpoint + `v3/forms?used-with-Yflow=`,
        headers: {
  ...
  async fetchUsers({ token }: { token: string }): Promise<KizeoFormsDataUsers> {
    const response = await httpClient.sendRequest<{
      ...
      method: HttpMethod.GET,
      url: endpoint + `v3/users?used-with-Yflow=`,
      headers: {
        'Content-Type': 'application/json',
        Authorization: token,
      },
  ...
  async fetchForm({
    ...
    const response = await httpClient.sendRequest<{ form: KizeoFormsForms }>({
      method: HttpMethod.GET,
      url: endpoint + `v3/forms/${formId}?used-with-Yflow=`,
      headers: {
        'Content-Type': 'application/json',
        Authorization: token,
      },
  ...
  async fetchExports({
    ...
    const response = await httpClient.sendRequest<{
      ...
      method: HttpMethod.GET,
      url: endpoint + `v3/forms/${formId}/exports?used-with-Yflow=`,
      headers: {
        'Content-Type': 'application/json',
        Authorization: token,
      },
  ...
  async fetchLists({ token }: { token: string }): Promise<KizeoFormsLists[]> {
    const response = await httpClient.sendRequest<{ lists: KizeoFormsLists[] }>(
      {
        method: HttpMethod.GET,
        url: endpoint + `v3/lists?used-with-Yflow=`,
        headers: {
          'Content-Type': 'application/json',
          Authorization: token,
        },
