...
export const askOpenAI = createAction({
  ...
  props: {
    model: Property.Dropdown({
      ...
      required: true,
      description:
        'The model which will generate completion. Some models are suitable for natural language tasks, others specialize in code.',
      refreshers: [],
    ...
    maxTokens: Property.Number({
      ...
      required: true,
      description:
        "The maximum number of tokens to generate. Requests can use up to 2,048 or 4,096 tokens shared between prompt and completion depending on model. Don't set the value to maximum and leave some tokens for the input. (One token is roughly 4 characters for normal English text)",
      defaultValue: 2048,
    }),
    ...
    memoryKey: Property.ShortText({
      ...
      description:
        'A memory key that will keep chat history shared across runs and flows. Keep it empty to leave ChatGPT without memory of previous messages.',
      required: false,
    }),
    roles: Property.Json({
      displayName: 'Roles',
      required: false,
      description: 'Array of roles to specify a more accurate response',
      defaultValue: [
