...
import { propsValidation, httpClient, HttpMethod } from '@yflow/pieces-common';
...
const optimizeFileProps = () => ({
  ...
  stored_file_id: Property.ShortText({
    ...
    description: 'ID of a previously stored file in Yflowto optimize',
    required: false,
  }),
  ...
  store_file: Property.Checkbox({
    ...
    description: 'Download and store the optimized file in yflow',
    required: false,
    defaultValue: true,
  }),
});
        ...
        try {
            ...
            if (import_method === 'url') {
                ...
                if (!context.server?.apiUrl) {
                    throw new Error('Server API URL is not available. Please check your Yflowserver configuration.');
                }
