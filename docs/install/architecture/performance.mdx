...
1. Run Yflowwith PostgreSQL and Redis with the following environment variables:
...
<Tip>
  These benchmarks are based on running Yflowin `SANDBOX_CODE_ONLY` mode. This does **not** represent the performance of YflowCloud, which uses a different sandboxing mechanism to support multi-tenancy. For more information, see [Sandboxing](/install/architecture/workers#sandboxing).
</Tip>
