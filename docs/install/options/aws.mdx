---
title: "AWS (Pulumi)"
description: "Get Yflowup & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy Yflowin a few clicks.
The template will deploy the latest Yflowimage that's available on [Docker Hub](https://hub.docker.com/r/yflow/yflow).
...
Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy Yflowdirect from Docker Hub using the template below in just few commands:
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your Yflowdeployment.

For a full list of all the currently available configuration options, take a look at the [YflowPulumi template file on GitHub](https://github.com/yflow/yflow/tree/main/deploy/pulumi/Pulumi.yaml).
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our Yflowdeployment:
...
```
6. Next we're going to use the YflowPulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
The config value of interest here is the `yflowUrl` as that is the URL for our Yflowdeployment.
...
Congratulations! You have successfully deployed Yflowto AWS.

## Deploy a locally built YflowDocker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the Yflowrepo locally:
...
Congratulations! You have successfully deployed Yflowinto AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official YflowDocker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.
