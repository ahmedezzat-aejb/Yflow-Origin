...
This documentation explains how to embed the Yflowiframe inside your application and customize it.
...
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<yflow_url>`. For example if the prefix is `/automation` and the Yflowurl is `/flows` the full url would be `/automation/flows`. |
...
| navigation.handler  | ❌ | `({route:string}) => void`  | This callback will be triggered each time a route in Yflowchanges, you can read more about it [here](/embedding/navigation) |
<Tip>
