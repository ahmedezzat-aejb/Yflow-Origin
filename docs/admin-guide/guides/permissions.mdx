...
Yflowutilizes Role-Based Access Control (RBAC) for managing permissions within projects. Each project consists of multiple flows and users, with each user assigned specific roles that define their actions within the project.

## Default Roles

Yflowcomes with four standard roles out of the box. The table below shows the permissions for each role:
