...
**Pieces** are the building blocks of Yflow— they are integrations and connectors (like Google Sheets, Slack, OpenAI, etc.) that users can use in their automation flows.
...
Platform administrators can manage pieces for the entire Yflowinstance from **Platform Admin → Setup → Pieces**.
...
<Steps>
  <Step title="Build Your Piece">
    Build your piece using the YflowCLI:
    ```bash
