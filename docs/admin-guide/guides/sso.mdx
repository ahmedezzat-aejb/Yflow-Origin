...
description: "Configure Single Sign-On (SSO) to enable secure, centralized authentication for your Yflowplatform"
icon: 'key'
---
...
Single Sign-On (SSO) allows your team to authenticate using your organization's existing identity provider, eliminating the need for separate Yflowcredentials. This improves security, simplifies user management, and provides a seamless login experience.

## Prerequisites

Before configuring SSO, ensure you have:

- **Admin access** to your Yflowplatform
- **Admin access** to your identity provider (Google, GitHub, Okta, or JumpCloud)
- The **redirect URL** from your YflowSSO configuration screen

## Accessing SSO Configuration

Navigate to **Platform Settings** → **SSO** in your Yflowadmin dashboard to access the SSO configuration screen.
...
Yflowsupports multiple SSO providers to integrate with your existing identity management system.
...
<Steps>
    ...
    <Step title="Configure Redirect URI">
        Copy the **Redirect URL** from the YflowSSO configuration screen and add it to the **Authorized redirect URIs** in Google Cloud Console.
    </Step>
    <Step title="Copy Credentials to yflow">
        Copy the **Client ID** and **Client Secret** from Google and paste them into the corresponding fields in yflow.
    </Step>
...
<Steps>
    ...
    <Step title="Register New Application">
        ...
        - **Application name**: Choose a recognizable name (e.g., "YflowSSO")
        - **Homepage URL**: Enter your Yflowinstance URL
    </Step>
    <Step title="Configure Authorization Callback">
        Copy the **Redirect URL** from the YflowSSO configuration screen and paste it into the **Authorization callback URL** field.
    </Step>
    ...
    <Step title="Copy Credentials to yflow">
        Copy the **Client ID** and **Client Secret** and paste them into the corresponding fields in yflow.
    </Step>
    <Step title="Save Configuration">
        Click **Finish** to complete the setup.
    </Step>
...
<Steps>
    ...
    <Step title="Configure General Settings">
        Enter an **App name** (e.g., "yflow") and optionally upload a logo. Click **Next**.
    </Step>
    <Step title="Configure SAML Settings">
        - **Single sign-on URL**: Copy the SSO URL from the Yflowconfiguration screen
        - **Audience URI (SP Entity ID)**: Enter `yflow`
...
<Steps>
    ...
    <Step title="Configure ACS URL">
        Copy the **ACS URL** from the Yflowconfiguration screen and paste it into the **ACS URLs** field in JumpCloud.

        ![JumpCloud ACS URL](/resources/screenshots/jumpcloud/acl-url.png)
    </Step>
    <Step title="Configure SP Entity ID">
        Set the **SP Entity ID** (Audience URI) to `yflow`.
    </Step>
    ...
    <Step title="Configure IdP Metadata in yflow">
        Paste the exported metadata XML into the **IdP Metadata** field in yflow.
    </Step>
...
If you encounter issues during SSO setup, please contact our enterprise support or [sales team](https://www.yflow.com/sales).
